...
func (m *Monitor) GetInfo() (map[string]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get Redis info: %s", err.Error())
	}
...
func (m *Monitor) GetMemoryUsage() (map[string]interface{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get memory info: %s", err.Error())
	}

	// 获取数据库大小
	dbSize, err := m.client.DBSize(ctx).Result()
	if err != nil {
		return nil, fmt.Errorf("failed to get database size: %s", err.Error())
	}

	// 获取连接数
	clientList, err := m.client.ClientList(ctx).Result()
	if err != nil {
		return nil, fmt.Errorf("failed to get client list: %s", err.Error())
	}
...
func (m *Monitor) GetKeyspaceStats() (map[string]interface{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get keyspace info: %s", err.Error())
	}
...
func (m *Monitor) GetPerformanceMetrics() (map[string]interface{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get command stats: %s", err.Error())
	}

	// 获取延迟统计
	latencyStats, err := m.client.Info(ctx, "latencystats").Result()
	if err != nil {
		return nil, fmt.Errorf("failed to get latency stats: %s", err.Error())
	}
